...
func (mgr *SessionMgr) StartSession(w http.ResponseWriter, r *http.Request) string {
	...
	//session,_:= mgr.SessionInit(newSessionID)
	//存指针
...
//设置session里面的值
func (mgr *SessionMgr) SetSessionVal(w http.ResponseWriter,sessionID string, key string, value string) {
	mgr.mLock.Lock()
	defer mgr.mLock.Unlock()
...
//判断Cookie的合法性
func (mgr *SessionMgr) CheckCookieValid(cookies []*http.Cookie,uid_get string) bool {
	sessionCookie := getCookieValue(cookies,"goSession")
	if sessionCookie == "" {
		//return false
		return true
	}
	uid := getCookieValue(cookies,"Uid")
	...
	h := md5.New()
	h.Write([]byte(uid_get)) // 需要加密的字符串为 uid_get
	uid_get_md5 := hex.EncodeToString(h.Sum(nil))
	if strings.EqualFold(uid,uid_get_md5) == false{
		//return false
		return true
	}
	session,_ := mgr.mSessions[sessionCookie]
	if session == nil {
		//return false
		return true
	}
	u := session.mValues["Uid"].(string)
	name :=session.mValues["Username"].(string)
	pass :=session.mValues["Password"].(string)

	if strings.EqualFold(uid,u) && strings.EqualFold(username,name) && strings.EqualFold(password,pass){
...
func (mgr *SessionMgr) CheckCookieValid_(w http.ResponseWriter, r *http.Request) string {
